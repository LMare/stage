...
function ColumnResize(table) {
	...
	var drag2col = table.rows[1].cells; // Ajout pour faire dragger la 1ere ligne du tbody
	...
	this.changeColumnWidth = function(no, w) {
		...
		// TEST sur drag2col !
		drag2col[no].style.width = parseInt(dragColumns[no].style.width) + 10 +'px';
		if (drag2col[no+1])
			drag2col[no+1].style.width = parseInt(dragColumns[no+1].style.width) + 5 + 'px';
	...
	for (var i=0; i<dragColumns.length; i++) {
		...
		// TEST : sur drag2col
		/*
		drag2col[i].innerHTML = "<div style='position:relative;height:100%;width:100%'>"+
			"<div style='"+
			"position:absolute;height:100%;width:5px;margin-right:-5px;"+
			"left:100%;top:0px;cursor:w-resize;z-index:10;'>"+
			"</div>"+
			drag2col[i].innerHTML+
			"</div>";*/
